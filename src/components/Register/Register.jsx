...
export default function Register() {
  ...
  const schema = z.object({
    name: z.string().min(2, "Name must includes atleast 2 char").max(10,"max length is 10 chars"),
    email: z.email("invalid email"),
    password: z.string().regex(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/, "must includes 1 capital char "),
    rePassword: z.string(),
  ...
  function handleRegister(values) {
    setisLoading(true);
    axios.post("https://linked-posts.routemisr.com/users/signup",values).then((res)=>{if(res.data.message=="success"){
       setisLoading(false);
      navigate("/login")
  ...
  return <>
    ...
    <form onSubmit={handleSubmit(handleRegister)}
      className="max-w-md mx-auto my-12">
        {apiError && (
          <h1 className='text-center bg-red-500 text-white p-2 my-2 font-bold rounded-2'>{apiError}</h1>
        )}
      ...
      <div className="relative z-0 w-full mb-5 group">
        ...
        <label htmlFor="repassword" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Enter rePassword</label>
      ...
      <div className='flex gap-4'>
        <div className="flex items-center mb-4">
          <input id="male" type="radio" {...register("gender")} value="male" className="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600" checked />
          <label htmlFor="male" className="block ms-2  text-sm font-medium text-gray-900 dark:text-gray-300">
            Male
          </label>
        </div>
        <div className="flex items-center mb-4">
          <input id="female" type="radio"{...register("gender")} value="female" className="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600" checked />
